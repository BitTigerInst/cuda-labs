...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	Matrix dM = AllocateDeviceMatrix(M),
	       dN = AllocateDeviceMatrix(N),
	       dP = AllocateDeviceMatrix(P);
	CopyToDeviceMatrix(dM, M);
	CopyToDeviceMatrix(dN, N);

	dim3 blocksPerGrid(1, 1), threadsPerBlock(P.height, P.width);
	MatrixMulKernel<<<blocksPerGrid, threadsPerBlock>>>(dM, dN, dP);
